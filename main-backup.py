...
def update_user_details_to_mysql(connection, record_id, nome_cliente, email, unidade):
    try:
        ...
        print(f"[INFO] Dados atualizados com sucesso no Record ID: {record_id}")
        print(f"Detalhes do usuário atualizados com sucesso para Record ID: {record_id}")
...
async def analyze_sentiment(transcription: str = Query(...)):
    try:
        ...
        if output in ["positivo", "negativo", "neutro", "positive"]:
            sentiment = output
        else:
...
async def update_user_details(data: UserDetails):
    print(data)
    try:
        print("Dados recebidos no backend:", data.model_dump())  # Substitui `data.dict()`
